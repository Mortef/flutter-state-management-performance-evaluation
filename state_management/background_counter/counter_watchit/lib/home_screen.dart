import 'package:counter_watchit/helpers/counter_material.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  final controller = GetIt.instance.get<CounterManager>();
  ...
  void runAfterBuildComplete() async {
    ...
    for (var i = 0; i < 10; i++) {
      await controller.loop(warmUpAmount);
    }
    for (var i = 0; i < loopAmount; i++) {
      await controller.loop(countAmount);
...
class _InputWidgetState extends State<_InputWidget> {
  ...
  @override
  Widget build(BuildContext context) {
    final watchItController = GetIt.instance.get<CounterManager>();
...
class _CounterText extends WatchingWidget {
  const _CounterText();
  @override
  Widget build(BuildContext context) {
    final counter = watchValue((CounterManager c) => c.counter);
    return Text(
      'Loop count: $counter',
...
class _MsText extends WatchingWidget {
  const _MsText();
  @override
  Widget build(BuildContext context) {
    final milliseconds = watchValue((CounterManager c) => c.milliseconds);
    if (milliseconds == 0) return const SizedBox.shrink();
    return Text('Elapsed time: $milliseconds ms');
  }
}

class _Background extends WatchingWidget {
  const _Background();
  @override
  Widget build(BuildContext context) {
    final isOther = watchValue((CounterManager c) => c.isMilestoneReached);
